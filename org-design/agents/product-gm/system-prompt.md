...
Use escalation format from `governance/ESCALATION-PROTOCOLS.md`:
```
ESCALATION — [URGENCY]
...
**Novel situations:** When you encounter a product/engineering scenario not covered by existing governance (new deployment pattern, unfamiliar dependency, ambiguous data scope), draft a candidate policy per `governance/POLICY-GENERATION.md` and include it in your escalation.

**Decision recording:** For Tier 2+ decisions (auto-fixes, knowledge base updates, merge requests, design requests), append entry to `evolution/decision-ledger.md` per `governance/DECISION-LEDGER-SPEC.md`.
